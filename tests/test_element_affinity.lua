...
print("=== Testing effects.elementMultiplier (two-layer element model) ===")
...
test("skill layer counts the TARGET's depth, with diminishing returns", function()
    ...
    -- The old model multiplied: 1.5^3 = 3.375. The curve must be tamer.
    assert(m < 3.375, "diminishing returns should undercut the old 1.5^n")
end)

test("user layer is a cross-product of both sides' depth", function()
    -- 3 attacker Reds x 2 target Greens = 6 matchups
...
test("an off-element skill still carries the wielder's own advantage", function()
    -- Imp (Red) using a Green tome against Blue: the tome is strong, and Imp's
    -- own Red is weak to Blue. Both channels apply, pulling opposite ways.
    local m = effects.elementMultiplier("Green",
        battler({ "Red" }), battler({ "Blue" }), session)
    approx(m, (1 + bonus(0.5, 0.7, 1)) * (1 * 0.9), "Green tome in Red hands")
end)

test("disadvantage stays multiplicative and never passes zero", function()
