...
function M.finish()
    -- Keep storage semantics and repository hygiene in the canonical unittest
    -- entry point so local verification and hosted CI enforce the same invariants.
    local storageOk, storageErr = pcall(function()
        require("tests.test_authored_storage").run()
    end)
    if not storageOk then
        M.crashed("tests.test_authored_storage", storageErr)
    end
