...
runValidation = function()
    ...
    local function validateAssignments(val, ownerDesc, id, paramDef_key)
        check(type(val) == "table", ownerDesc .. " command '" .. id .. "' param '" .. paramDef_key .. "' expects a list of {name, value} rows")
        if type(val) == "table" then
            local formulaEngine = require("engine.formula")
            local mockCtx = buildFormulaMockCtx()
            for ai, a in ipairs(val) do
                check(type(a) == "table" and type(a.name) == "string" and a.name ~= "",
                    ownerDesc .. " command '" .. id .. "' " .. paramDef_key .. "[" .. ai .. "] needs a non-empty string name")
                if type(a) == "table" then
                    local ok, result, ferr = pcall(formulaEngine.eval, a.value, mockCtx)
                    check(ok and ferr == nil, ownerDesc .. " command '" .. id .. "' " .. paramDef_key .. "[" .. ai .. "] value failed to compile formula '" .. tostring(a.value) .. "': " .. tostring(ferr))
        ...
        for _, cmd in ipairs(cmds or {}) do
            ...
            if cmdDef then
                ...
                elseif paramDef.type == "assignments" then
                    ...
                    validateAssignments(val, ownerDesc, id, paramDef.key)
