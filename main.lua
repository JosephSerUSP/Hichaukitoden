...
local function populateShopPreviewState(loader, st)
    if not st or (st.v.items and #st.v.items > 0) then return end
...
local function runPreviewScene(sceneId)
    ...
    local ok, err = pcall(function()
        ...
        if tostring(sceneDef.id) == "shop" then
            populateShopPreviewState(loader, sh.getCurrentState())
        end
