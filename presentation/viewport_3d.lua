...
local function getCompositeTileCanvas(atlas, originX, originY, leftEdgeSpec, rightEdgeSpec, featureOverlay, wallOverlay)
    ...
    -- Canvas targets are not part of Lﾃ坊E's push/pop graphics state. Failing
...
-- (Lﾃ坊E requires every declared Image uniform to exist in the source, and
...
local function addWorldVertex(group, x, y, z, u, v, r, g, b, fogFactor)
    -- VertexColor feeds Lﾃ坊E's built-in `color` shader argument. Keep it
...
local function drawWorldSpace(session)
    ...
    local function addWall(mapX, mapY, kind, p1, p2, nx, ny)
        ...
        -- Lﾃ坊E image V=0 is the top edge.  These vertices are authored
...
function viewport_3d.draw(session)
    ...
    -- shader. The pre-3D raycaster this replaced is preserved in git history
    -- (see the AGENTS.md-linked renderer-3d-roadmap.md), not inline here.
