...
The original run published through GitHub object writes rather than a full repository checkout, so its `data/items.json` rewrite was missing and the runtime still fell back to the missing-model placeholder for IDs 53–62. The batch generator has since been re-run from a normal checkout (2026-08-08); the ten explicit assignments are committed and the models resolve at runtime.

That same object-write path also corrupted three of the committed OBJ files. Re-running the generator restored them: `adamant_blade.obj` and `hazel_wand.obj` differed only in float formatting, but `knight_sword.obj` carried a genuinely wrong vertex (`-0.033376` where the recipe produces `-0.033076`). The other seven regenerated byte-identically, which is what makes the three stand out as transport damage rather than recipe drift. Prefer a real checkout for any future batch: the generator is deterministic and its output is verifiable only when the bytes survive intact.
