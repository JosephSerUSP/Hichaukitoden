...
$targetAspect = $NativeWidth / $NativeHeight
$cellAspect = $CellWidth / $CellHeight
if ($cellAspect -gt $targetAspect) {
    $cropWidth, $cropHeight = [math]::Floor($CellHeight * $targetAspect), $CellHeight
} else {
    $cropWidth, $cropHeight = $CellWidth, [math]::Floor($CellWidth / $targetAspect)
}
...
for ($i = 0; $i -lt $Names.Count; $i++) {
    ...
    & magick $inputItem.FullName `
        ...
        -filter Lanczos -resize "$($NativeWidth)x$($NativeHeight)!" `
        -colors 128 PNG8:$native
    if ($LASTEXITCODE -ne 0) { throw "Failed to resize '$($Names[$i])'." }
}

Write-Output "Split $($Names.Count) plates into '$OutputDirectory'."
